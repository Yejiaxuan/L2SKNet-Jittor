...
class LLSKM_1D(nn.Module):
    ...
    def execute(self, x):
        ...
        # 通过外积重构nxn核
        nxn_kernel = jt.zeros((kernel_1xn.shape[0], kernel_1xn.shape[1], self.kernel_size, self.kernel_size))
        ...
        out_center = jt.nn.conv2d(x, weight=nxn_kernel_sum, bias=self.conv_1xn.bias, stride=self.stride,
        ...
        out_offset = jt.nn.conv2d(x, weight=nxn_center_kernel, bias=self.conv_1xn.bias, stride=self.stride,
                                  padding=0, groups=self.groups)
